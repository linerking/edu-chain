...
def employer_login(request):
    ...
    if user is not None:
        ...
        if user.has_perm('auth.employer'):
          return redirect('https://blockchain-906870065650856.codeanyapp.com/draw/employer_search/')
        # 进行登录成功的操作，重定向到某处等
    else:
        # 返回用户名和密码错误信息
        return redirect('https://blockchain-906870065650856.codeanyapp.com/draw/login_employer/')
def school_login(request):
    ...
    if user is not None:
        ...
        if user.has_perm('auth.student'):
          return redirect('https://blockchain-906870065650856.codeanyapp.com/draw/student_main/')
        elif user.has_perm('auth.teacher'):
          return redirect('https://blockchain-906870065650856.codeanyapp.com/draw/teacher_main/')
        elif user.has_perm('auth.school'):
          return redirect('https://blockchain-906870065650856.codeanyapp.com/draw/school_main/')
        # 进行登录成功的操作，重定向到某处等
    else:
        # 返回用户名和密码错误信息
        return redirect('https://blockchain-906870065650856.codeanyapp.com/draw/login_school/')
def add_permission(request):
    ...
    student = models.Group.objects.filter(name='student').first()
